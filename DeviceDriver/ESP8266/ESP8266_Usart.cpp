...
int Step=0;
int AckComLen=0;
uint32_t ESP8266_TgtSysTick;
uint32_t ESP8266_SysTickCache;
void ESP8266_Init()
{
...
void ESP8266_WaitmS(uint32_t Ms)
{
...
uint8_t DelayIsOver()
{
	uint32_t NowSysTick=1;//GetSysTickLow();
...
void ESP8266_ATCmdSend(const char *Cmd,const char *Ack,int AckLen,uint32_t WaitMs)
{
...
static float Pm25Data;
static float TempData;
static float HumiData;
void ESP8266_UpdateData(float UpdateData1,float UpdateData2,float UpdateData3)
{
	Pm25Data=UpdateData1;
	TempData=UpdateData2;
	HumiData=UpdateData3;
}
void ESP8266_UpLoadData()
{
	ESP8266_Ds18b20UpdateLeWei50(Pm25Data,TempData,HumiData);
}
void ESP8266_Ds18b20UpdateLeWei50(float UpdateData1,float UpdateData2,float UpdateData3)
{
		static float UpdateDataCache;
	//static uint8_t Step=0;
  static char CmdBuf[200]={0};
	static char Cmd2Buf[200]={0};
	int i;
	//if(DelayIsOver()){
		DebugStep(Step);
		if(0x00==Step){
		...
		if(0x06==Step){
			UpdateDataCache=UpdateData1;
			sprintf(Cmd2Buf,"{\"method\":\"upload\",\"data\":[{\"Name\":\"PM25\",\"Value\":\"%.1f\"},{\"Name\":\"DHT21T\",\"Value\":\"%0.1f\"},{\"Name\":\"DHT21H\",\"Value\":\"%0.1f\"}]}&^!",UpdateData1,UpdateData2,UpdateData3);
			i=strlen(Cmd2Buf);
			sprintf(CmdBuf,"AT+CIPSEND=%d\r\n",i);
			ESP8266_ATCmdSend(CmdBuf,"OK\r\n> ",6,3000);
